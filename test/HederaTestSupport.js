...
async function deploy(tokenJson, parameters, gas) {
  ...
  if (parameters.length > 0) {
    ...
    const constructParameterAsHexString = abiInterface
      .encodeDeploy(parameters)
      .slice(2);
    // convert to a Uint8Array
    const constructorParametersAsUint8Array = Buffer.from(
      constructParameterAsHexString,
      "hex"
    );

    contractTransaction.setConstructorParameters(
      constructorParametersAsUint8Array
    );
  }
...
async function call(functionName, parameters, gas) {
  ...
  const results = abiInterface.decodeFunctionResult(
    functionName,
